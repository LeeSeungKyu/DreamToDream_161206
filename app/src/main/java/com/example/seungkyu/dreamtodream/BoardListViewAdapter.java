...
public class BoardListViewAdapter extends BaseAdapter {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        writerTV.setText(boardVO.getAuthor());
        dateTV.setText(boardVO.getDate());

        return convertView;
    }

    public void addItem(String title, String author, String date) { // DB에서 가져올 때 게시물 pk 값도 가져와야할듯 이걸로 본문 조회해서 본문만 가져오고 나머지는 인텐트.
        BoardVO boardVO = new BoardVO(title, author, date);
